...
#include "SENSOR_H/imx385_wdr30.h"
#include "SENSOR_H/imx326_default_v5.h"
#include "SENSOR_H/imx335_default_v5.h"
...
struct isp_cfg_array cfg_arr[] = {
	...
	{"imx317_mipi", "imx317_default_ini_4v", 1920, 1080, 30, 0, 0, &imx317_default_ini_4v5},
	...
	{"imx385_mipi", "imx385_wdr30", 1920, 1080, 30, 1, 0, &imx385_mipi_isp_cfg},
	{"imx335_mipi", "imx335_default_v5", 1920, 1920, 30, 0, 0, &imx335_mipi_isp_cfg},
};
...
int parser_sync_info(struct isp_param_config *param, char *isp_cfg_name, int isp_id)
{
	...
	int lsc_temp = 0;
	int enable = 0;
	sprintf(fdstr, "/mnt/SDCARD/%s_%d.bin", isp_cfg_name, isp_id);
			...
			int w, int h, int fps, int wdr, int ir, int sync_mode, int isp_id)
{
	...
	if (i == array_size(cfg_arr)) {
		...
		if (!strncmp(sensor_name, "imx317", 6)) {
			...
			cfg = &imx258_isp_cfg;
		} else if (!strncmp(sensor_name, "imx385", 6)) {
			ISP_PRINT("use imx385 isp config.\n");
			cfg = &imx385_mipi_isp_cfg;
		} else if (!strncmp(sensor_name, "imx326", 6)) {
			ISP_PRINT("use imx326 isp config.\n");
			cfg = &imx326_mipi_isp_cfg;
		} else if (!strncmp(sensor_name, "imx335", 6)) {
			ISP_PRINT("use imx335 isp config.\n");
			cfg = &imx335_mipi_isp_cfg;
		} else {
