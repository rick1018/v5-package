...
int mpp_comm_ise_get_cfg(ISE_MODE_CFG_E ise_type, int chn_num, ISE_CHN_ATTR_S *chn_attr)
{
    ...
    switch (ise_type)
    {
        ...
        case ISE_FISH_TOP_4PTZ_MODE_GRP0_1920_TO_960x960:
        case ISE_FISH_TOP_4PTZ_MODE_GRP1_1920_TO_960x960:
        case ISE_FISH_TOP_4PTZ_MODE_GRP2_1920_TO_960x960:
        case ISE_FISH_TOP_4PTZ_MODE_GRP3_1920_TO_960x960:
        {
            chn_attr->mode_attr.mFish.ise_cfg.dewarp_mode     = WARP_NORMAL;
            chn_attr->mode_attr.mFish.ise_cfg.mount_mode      = MOUNT_TOP;
            chn_attr->mode_attr.mFish.ise_cfg.in_w            = 1920;
            chn_attr->mode_attr.mFish.ise_cfg.in_h            = 1920;
            chn_attr->mode_attr.mFish.ise_cfg.in_luma_pitch   = 1920;
            chn_attr->mode_attr.mFish.ise_cfg.in_chroma_pitch = 1920;
            chn_attr->mode_attr.mFish.ise_cfg.in_yuv_type     = 0;
            chn_attr->mode_attr.mFish.ise_cfg.out_yuv_type    = 0;
            chn_attr->mode_attr.mFish.ise_cfg.p               = 1920/3.1415;
            chn_attr->mode_attr.mFish.ise_cfg.cx              = 1920/2;
            chn_attr->mode_attr.mFish.ise_cfg.cy              = 1920/2;
            for (i = 0; i < chn_num; i++)
            {
                chn_attr->mode_attr.mFish.ise_cfg.out_en[i]           = 1;
                chn_attr->mode_attr.mFish.ise_cfg.out_w[i]            = 960;
                chn_attr->mode_attr.mFish.ise_cfg.out_h[i]            = 960;
                chn_attr->mode_attr.mFish.ise_cfg.out_flip[i]         = 1;
                chn_attr->mode_attr.mFish.ise_cfg.out_mirror[i]       = 0;
                chn_attr->mode_attr.mFish.ise_cfg.out_luma_pitch[i]   = 960; /* pitch%32 == 0*/
                chn_attr->mode_attr.mFish.ise_cfg.out_chroma_pitch[i] = 960;
            }
            chn_attr->mode_attr.mFish.ise_cfg.mount_mode = MOUNT_TOP;
            chn_attr->mode_attr.mFish.ise_cfg.tilt       = 45;
            chn_attr->mode_attr.mFish.ise_cfg.zoom       = 2;
            if (ISE_FISH_TOP_4PTZ_MODE_GRP0_1920_TO_960x960 == ise_type)
            {
                chn_attr->mode_attr.mFish.ise_cfg.pan = 0; /* 45, 90, 135, 225 */
            }
            else if (ISE_FISH_TOP_4PTZ_MODE_GRP1_1920_TO_960x960 == ise_type)
            {
                chn_attr->mode_attr.mFish.ise_cfg.pan = 90; /* 45, 90, 135, 225 */
            }
            else if (ISE_FISH_TOP_4PTZ_MODE_GRP2_1920_TO_960x960 == ise_type)
            {
                chn_attr->mode_attr.mFish.ise_cfg.pan = 180; /* 45, 90, 135, 225 */
            }
            else if (ISE_FISH_TOP_4PTZ_MODE_GRP3_1920_TO_960x960 == ise_type)
            {
                chn_attr->mode_attr.mFish.ise_cfg.pan = 270; /* 45, 90, 135, 225 */
            }
        }
