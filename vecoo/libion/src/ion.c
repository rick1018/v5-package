...
unsigned int ion_get_phyaddr(int fd, ion_user_handle_t handle, size_t size, unsigned int *pPhyAddr)
{
    int ret = 0;
    ...
    if (handle <= 0 || size <= 0) {
        ALOGE("user hanlde error\n");
        return 0;
    }
    if (NULL == pPhyAddr) {
        ALOGE("phyaddr ptr null\n");
        return 0;
    }
...
int ion_flush_cache(int fd, void* pVirAddr, unsigned int size)
{
    ...
    range.start = (unsigned long)pVirAddr;
    range.end = range.start + size;

    ret = ion_ioctl(fd, ION_IOC_SUNXI_FLUSH_RANGE, &range);
    if (ret) 
    {
