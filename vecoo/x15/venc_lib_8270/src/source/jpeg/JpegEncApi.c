                         ...
                         JpegEncOut * pEncOut)
{
    ...
    else
    {
        ...
        {
            preProcess->lumHeight = (16 * ((i32) pEncInst->jpeg.restart.Ri /
                                           ((pEncInst->jpeg.width + 15) / 16)));
        }
        else
        {
            preProcess->lumHeight = (jpeg->height -
                                     (jpeg->sliceNum * (jpeg->sliceRows * 16)));
        }
...
bool_e CheckJpegCfg(const JpegEncCfg * pEncCfg)
{
    ...
    if(pEncCfg->frameType != JPEGENC_YUV420_PLANAR &&
       pEncCfg->frameType != JPEGENC_YUV420_SEMIPLANAR &&
       pEncCfg->frameType != JPEGENC_YUV422_INTERLEAVED_YUYV &&
       pEncCfg->frameType != JPEGENC_YUV422_INTERLEAVED_UYVY &&
       pEncCfg->frameType != JPEGENC_RGB565 &&
       pEncCfg->frameType != JPEGENC_BGR565 &&
       pEncCfg->frameType != JPEGENC_RGB555 &&
       pEncCfg->frameType != JPEGENC_BGR555 &&
       pEncCfg->frameType != JPEGENC_RGB444 &&
       pEncCfg->frameType != JPEGENC_BGR444 &&
       pEncCfg->frameType != JPEGENC_RGB888 &&
       pEncCfg->frameType != JPEGENC_BGR888 &&
       pEncCfg->frameType != JPEGENC_RGB101010 &&
