...
test_set() {
    ...
    if [ ${valid[${set_nro}]} -eq -1 ]; then
	    ...
	    if [ -e random_run ]
	    then
	        mv video_stab_result.log ${TESTDIR}/vs_case_${case_nro}/vs_case_$case_nro.trc
		#mv case_${case_nro}.time random_cases/case_${case_nro}.time
            ...
            if ( [ "$TRACE_EWL" == "y" ] )
            then
                mv ewl.trc $TESTDIR/vs_case_${case_nro}/ewl.trc
            fi

            touch ${TESTDIR}/vs_case_${case_nro}/run_${case_nro}_stab_done
