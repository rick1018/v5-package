...
while [ ! -e ${TESTDIR}/vs_case_${case_nro}/run_${case_nro}_stab_done ]
do
    sleep 1    
...
(
    ...
    echo -e -n "Stabilization Case $case_nro\t"
           
    # Compare output to reference
    if (cmp -s ${TESTDIR}/vs_case_${case_nro}/vs_case_$case_nro.trc ${case_dir}/video_stab_result.trc)
    then
        echo "OK"
        exit 0
    else
