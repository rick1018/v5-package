...
int infotm_ion_get_phys(int fd, int share_fd, unsigned int *phy_addr)
{
	...
	if (!(infotm_data = malloc(sizeof(struct infotm_ion_buffer_data)))) {
		ALOGE("Could not malloc infotm_ion_buffer_data");
		return -1;
	}

	infotm_data->fd = share_fd;
	...
	if (ret<0) {
		ALOGE("infotm_ion_get_phys failed in ioctl: %d", ret);
	}
	else
		*phy_addr = infotm_data->phy_addr;
