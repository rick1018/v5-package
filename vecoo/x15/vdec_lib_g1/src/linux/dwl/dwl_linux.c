...
u32 DWLReadAsicID()
{
    ...
    io = DWLMapRegisters(fd_dec, base, regSize, 0);
...
i32 DWLMallocLinear(const void *instance, u32 size, DWLLinearMem_t * info)
{
    ...
    if(dec_dwl->mmu_enable)
    {
         ...
         {
		     DWL_DEBUG("DWLMallocLinear: ERROR! can not get dev addr \n");
             return DWL_ERROR;
         }
    ...
    DWL_DEBUG("DWLMallocLinear:: phy= %x, vir =%p \n", info->busAddress,info->virtualAddress);
...
void DWLEnableHW(const void *instance, u32 offset, u32 value)
{
	...
	if(dec_dwl->dvfsEna  && !dec_dwl->hwEna){
		dec_dwl->hwEna = true;
		if(ioctl(dec_dwl->fd, VDEC_NOTIFY_HW_ENABLE) != 0){
...
i32 DWLMMUEnable(const void *instance, int enable)
{
	hX170dwl_t *dec_dwl = (hX170dwl_t *)instance;
    int mmu_inited = 0; 

    ioctl(dec_dwl->fd, HX170DEC_CHECK_MMU_INIT, &mmu_inited);

    if(enable == 1 && mmu_inited == 0)
    {
        return -1;
    }

    dec_dwl->mmu_enable = enable;
