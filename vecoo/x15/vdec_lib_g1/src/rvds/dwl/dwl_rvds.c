...
i32 DWLMallocLinear(const void *instance, u32 size, DWLLinearMem_t * info)
{
	...
	//DWLmemset(info->virtualAddress,0,info->size);
    return DWL_OK;
}
...
void DWLWriteReg(const void *instance, u32 offset, u32 value)
{
    ...
    *(dec_dwl->pRegBase + offset) = value;
	...
	vdec_rt_write_reg((unsigned int)(dec_dwl->pRegBase + offset), value);
}
...
u32 DWLReadReg(const void *instance, u32 offset)
{
	...
	vdec_rt_read_reg((unsigned int)(dec_dwl->pRegBase + offset), val);
...
void DWLEnableHW(const void *instance, u32 offset, u32 value)
{
	...
	if(offset == 4){	// 0x04--decoder, 0xf0--pp.
		vdec_rt_enable_hw();
	}
...
{
    return mm_module_malloc(MODULE_VDEC, n,MM_ALLOC_ALIGN_8BYTE);
}
...
{
    return mm_module_malloc(MODULE_VDEC, n*s,MM_ALLOC_ALIGN_8BYTE);
}
