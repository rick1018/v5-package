...
u32 RunDecoderAsic(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    else    /* in the middle of decoding, continue decoding */
    {
        ...
        DWLWriteReg(pDecContainer->dwl, 4 * 5, pDecContainer->avsRegs[5]);
        DWLWriteReg(pDecContainer->dwl, 4 * 6, pDecContainer->avsRegs[6]);
        DWLWriteReg(pDecContainer->dwl, 4 * 12, pDecContainer->avsRegs[12]);

        DWLWriteReg(pDecContainer->dwl, 4 * 1, pDecContainer->avsRegs[1]);
    }
                                ...
                                AvsDecPicture * pPicture, u32 endOfStream)
{
    ...
    {
        (void) DWLmemset(pPicture, 0, sizeof(AvsDecPicture));
...
/*------------------------------------------------------------------------------
    ...
    Function name: AvsDecMMUEnable

    Functional description:
        configure decoder to use buffer allocated through mmu
...
------------------------------------------------------------------------------*/
AvsDecRet AvsDecMMUEnable(AvsDecInst decInst, int mmu_enable)
{
