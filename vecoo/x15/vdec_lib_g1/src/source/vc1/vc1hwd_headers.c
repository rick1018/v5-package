                               ...
                               strmData_t *pStrmData )
{
    ...
    if( pStorage->hrdParamFlag )
    {
        ...
        if (pStorage->hrdRate)
        {
            DWLfree(pStorage->hrdRate);
            pStorage->hrdRate = NULL;
        }
        if (pStorage->hrdBuffer)
        {
            DWLfree(pStorage->hrdBuffer);
            pStorage->hrdBuffer = NULL;
        }
        pStorage->hrdRate = 
                    DWLmalloc( pStorage->hrdNumLeakyBuckets * sizeof(u32) );
        pStorage->hrdBuffer = 
                    DWLmalloc( pStorage->hrdNumLeakyBuckets * sizeof(u32) );
		...
		DWLmemset(pStorage->hrdRate,0,pStorage->hrdNumLeakyBuckets * sizeof(u32));
		DWLmemset(pStorage->hrdBuffer,0,pStorage->hrdNumLeakyBuckets * sizeof(u32));
...
/* Code */
    ...
    if (pStorage->hrdParamFlag)
    {
        ...
        if(pStorage->hrdFullness == NULL)
            return (VC1HWD_MEMORY_FAIL);
		DWLmemset(pStorage->hrdFullness,0,pStorage->hrdNumLeakyBuckets * sizeof(u32));
