...
u32 VC1RunAsic(decContainer_t *pDecCont, strmData_t *pStrm, u32 strmBusAddr)
{
    ...
    if (!pDecCont->asicRunning)
    {
        ...
        DWLWriteReg(pDecCont->dwl, 0x4, 0);

        for (i = 2; i < DEC_X170_REGISTERS; i++)
        {
            DWLWriteReg(pDecCont->dwl, 4*i, pDecCont->vc1Regs[i]);
    ...
    else /* buffer was empty and now we restart with new stream values */
    {
        ...
        DWLWriteReg(pDecCont->dwl, 4 * 5, pDecCont->vc1Regs[5]);
        DWLWriteReg(pDecCont->dwl, 4 * 6, pDecCont->vc1Regs[6]);
        DWLWriteReg(pDecCont->dwl, 4 * 12, pDecCont->vc1Regs[12]);
    ...
    {
        pDecCont->vc1Regs[i] = DWLReadReg(pDecCont->dwl, 4*i);
    }
