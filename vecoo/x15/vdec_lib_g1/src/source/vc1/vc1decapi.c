...
void VC1DecRelease(VC1DecInst decInst)
{
    ...
    if (pDecCont->storage.hrdRate)
        DWLfree(pDecCont->storage.hrdRate);
    if (pDecCont->storage.hrdBuffer)
        DWLfree(pDecCont->storage.hrdBuffer);
    if (pDecCont->storage.hrdFullness)
        DWLfree(pDecCont->storage.hrdFullness);
...
/* Code */
    ...
    if(pDecCont->asicRunning)
    {
        DWLWriteReg(pDecCont->dwl, 1 * 4, 0);   /* stop HW */
...
/* Code */
    ...
    if( pDecCont->storage.outpCount == 0 ||
        ( pDecCont->storage.picLayer.fcm == FIELD_INTERLACE &&
          pDecCont->storage.picLayer.isFF ) )
    {
        DWLmemset(pPicture, 0, sizeof(VC1DecPicture));
        return VC1DEC_OK;
    }
...
/*------------------------------------------------------------------------------
    ...
    Return values:
        VC1DEC_OK               MMU Configed.
        VC1DEC_INITFAIL         MMU Config Failed
-----------------------------------------------------------------------------------*/
VC1DecRet VC1DecMMUEnable(VC1DecInst decInst, int mmu_enable)
{
