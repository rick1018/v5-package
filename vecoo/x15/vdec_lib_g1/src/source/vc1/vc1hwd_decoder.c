    ...
    u32 numFrameBuffers )
{
    ...
    (void)DWLmemset(pStorage, 0, sizeof(swStrmStorage_t));
                       ...
                       const void *dwl )
{
    ...
    pPic = (picture_t*)DWLmalloc(sizeof(picture_t) * pStorage->workBufAmount);
    if(pPic == NULL)
    {
        (void)vc1hwdRelease(dwl, pStorage);
        return (VC1HWD_MEMORY_FAIL);
    }
    (void)DWLmemset(pPic, 0, sizeof(picture_t)*pStorage->workBufAmount);
    ...
    {
        if (DWLMallocRefFrm(dwl, size, &pPic[i].data) != 0)
        {
            (void)vc1hwdRelease(dwl, pStorage);
            return (VC1HWD_MEMORY_FAIL);
        }
        (void)DWLmemset(pPic[i].data.virtualAddress, 0 , size);
    ...
    pStorage->pMbFlags = (u8*)DWLmalloc(((pStorage->numOfMbs+9)/10)*10);
    if (pStorage->pMbFlags == NULL)
    {
        (void)vc1hwdRelease(dwl, pStorage);
        return (VC1HWD_MEMORY_FAIL);
    }
    (void)DWLmemset(pStorage->pMbFlags, 0, ((pStorage->numOfMbs+9)/10)*10);
    ...
    if(pDecCont->storage.maxBframes)
    {
        ...
        {
            rv = DWLMallocLinear(pDecCont->dwl,
...
/* Code */
    ...
    if (pPic != NULL)
    {
        ...
        DWLfree((picture_t*)pStorage->pPicBuf);

        pStorage->pPicBuf = NULL;
    }
    if(pStorage->pMbFlags)
    {
        DWLfree(pStorage->pMbFlags);
