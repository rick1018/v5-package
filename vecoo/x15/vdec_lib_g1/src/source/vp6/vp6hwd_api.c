...
VP6DecRet VP6DecPeek(VP6DecInst decInst, VP6DecPicture * pOutput)
{
    ...
    {
        (void)DWLmemset(pOutput, 0, sizeof(VP6DecPicture));
        return VP6DEC_OK;
    }
...
/*------------------------------------------------------------------------------
    Function name   : VP6DecMMUEnable
    Description     : 
    Return type     : VP6DecRet 
    Argument        : VP6DecInst decInst mmu_enable
------------------------------------------------------------------------------*/
VP6DecRet VP6DecMMUEnable(VP6DecInst decInst, int mmu_enable)
{
    VP6DecContainer_t *pDecCont = (VP6DecContainer_t *) decInst;
    if(DWLMMUEnable(pDecCont->dwl, mmu_enable) == -1)
    {
        return VP6DEC_INITFAIL;
    }
