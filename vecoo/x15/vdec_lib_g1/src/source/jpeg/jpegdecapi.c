...
JpegDecRet JpegDecInit(JpegDecInst * pDecInst)
{
    ...
    *pDecInst = NULL;   /* return NULL instance for any error */
	...
	DWLmemset(pJpegCont,0,sizeof(JpegDecContainer));
    pJpegCont->dwl = dwl;
...
#ifdef INFOTM_JPEG_FIND_MARK
	u32 infotmCurMark = 0;
	u32 infotmNextMark = 0;
	u8 *previousPos = stream.pCurrPos;
	u32 previousReadBits = stream.readBits;
	do{
		infotmCurMark = JpegDecGetByte(&(stream));
		if(infotmCurMark == STRM_ERROR){
			if(!PTR_JPGC->info.init)
				return JPEGDEC_INCREASE_INPUT_BUFFER;
			else
				return JPEGDEC_STRM_ERROR;
		}
		previousPos = stream.pCurrPos;
		previousReadBits = stream.readBits;
		if(infotmCurMark == 0xFF){
			infotmNextMark = 	JpegDecGetByte(&(stream));
			if(infotmNextMark == STRM_ERROR){
				if(!PTR_JPGC->info.init)
					return JPEGDEC_INCREASE_INPUT_BUFFER;
				else
					return JPEGDEC_STRM_ERROR;				
			}
			if(infotmNextMark>= 0xC0 && infotmNextMark <= 0xFE){
				stream.pCurrPos = previousPos;
				stream.readBits = previousReadBits;
				break;
			}else{
				stream.pCurrPos = previousPos;
				stream.readBits = previousReadBits;
            ...
            case APP0:
                ...
                if(thumbnail)
                {
                    ...
                    if(currentByte == JPEGDEC_THUMBNAIL_JPEG)
                    {
                        ...
                        {
#ifdef INFOTM_JPEG_FIND_MARK
				u32 infotmCurMarkTn = 0;
				u32 infotmNextMarkTn = 0;
				u8 *previousPosTn = NULL;
				u32 previousReadBitsTn = 0;
				do{
					infotmCurMarkTn = JpegDecGetByte(&(stream));
					appBits += 8;
					if(infotmCurMarkTn == STRM_ERROR){
						if(!PTR_JPGC->info.initThumb &&pDecIn->bufferSize)
							return JPEGDEC_INCREASE_INPUT_BUFFER;
						else
							return JPEGDEC_STRM_ERROR;
					}
					previousPosTn = stream.pCurrPos;
					previousReadBitsTn = stream.readBits;
					if(infotmCurMarkTn == 0xFF){
						infotmNextMarkTn = 	JpegDecGetByte(&(stream));
						if(infotmNextMark == STRM_ERROR){
							if(!PTR_JPGC->info.initThumb && pDecIn->bufferSize)
								return JPEGDEC_INCREASE_INPUT_BUFFER;
							else
						...
						if(infotmNextMark>= 0xC0 && infotmNextMark <= 0xFE){
							stream.pCurrPos = previousPosTn;
							stream.readBits = previousReadBitsTn;
							break;
						}else{
							stream.pCurrPos = previousPosTn;
							stream.readBits = previousReadBitsTn;
...
#ifdef INFOTM_SUPPORT_EXTRA_THUMBNAIL
			else if(currentByte == JPEGENC_THUMB_RGB24 || 
                    				currentByte == JPEGENC_THUMB_PALETTE_RGB8){
				if(currentByte == JPEGENC_THUMB_RGB24)
					pImageInfo->thumbnailType = JPEGENC_THUMB_RGB24;
				else 
					pImageInfo->thumbnailType = JPEGENC_THUMB_PALETTE_RGB8;
				pImageInfo->outputWidthThumb  =
					pImageInfo->displayWidthThumb= JpegDecGetByte(&(stream));
				pImageInfo->outputHeightThumb =
					pImageInfo->displayHeightThumb = JpegDecGetByte(&(stream));
				pImageInfo->codingModeThumb = JPEGDEC_BASELINE;

				pImageInfo->pThumbnailDataVirtual = stream.pCurrPos;
				pImageInfo->pThumbnailDataBus =pDecIn->streamBuffer.busAddress + 	\
...
#ifdef INFOTM_JPEG_FIND_MARK
	do{
		infotmCurMark = JpegDecGetByte(&(PTR_JPGC->stream));
		if(infotmCurMark == STRM_ERROR){
			shouldBreak = 1;
			break;
		}
		previousPos = PTR_JPGC->stream.pCurrPos;
		previousReadBits = PTR_JPGC->stream.readBits;
		if(infotmCurMark == 0xFF){
			infotmNextMark = 	JpegDecGetByte(&(PTR_JPGC->stream));
			if(infotmNextMark == STRM_ERROR){
				shouldBreak = 1;
				break;
			}
			if(infotmNextMark>= 0xC0 && infotmNextMark <= 0xFE){
				PTR_JPGC->stream.pCurrPos = previousPos;
				PTR_JPGC->stream.readBits = previousReadBits;
				break;
			}else{
				PTR_JPGC->stream.pCurrPos = previousPos;
				PTR_JPGC->stream.readBits = previousReadBits;
    ...
    if(PTR_JPGC->image.headerReady)
    {
        ...
        do
        {
            ...
            else if(dwlret == DWL_HW_WAIT_TIMEOUT)
            {
                ...
                /* check PP status... and go */
				/* added by ayakashi to fixed jpeg dec timeout bug*/
                if(PTR_JPGC->ppInstance != NULL)
                {
                   /* End PP co-operation */
                   if(PTR_JPGC->ppControl.ppStatus == DECPP_RUNNING)
...
/*------------------------------------------------------------------------------
    5.9. Function name   : jpegDecMMUEnable
         Description     : config decoder to use buffers allocated through mmu
         Return type     : 
         Argument        : const void * decInst - decoder instance
------------------------------------------------------------------------------*/
JpegDecRet  JpegDecMMUEnable(JpegDecInst decInst, int mmu_enable)
{
    JpegDecContainer *pDecCont;

    pDecCont = (JpegDecContainer *) decInst;

    if(DWLMMUEnable(pDecCont->dwl, mmu_enable) == -1)
    {
        return JPEGDEC_DWL_ERROR;
    }
