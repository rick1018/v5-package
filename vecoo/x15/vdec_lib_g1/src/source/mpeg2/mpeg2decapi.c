...
#define MPEG2DEC_UPDATE_POUTPUT \
    ...
    (void) DWLmemcpy(pOutput, &pDecCont->MbSetDesc.outData, \
...
u32 RunDecoderAsic(DecContainer * pDecContainer, u32 strmBusAddress)
{
    ...
    else    /* in the middle of decoding, continue decoding */
    {
        ...
        DWLWriteReg(pDecContainer->dwl, 4 * 5, pDecContainer->mpeg2Regs[5]);
        DWLWriteReg(pDecContainer->dwl, 4 * 6, pDecContainer->mpeg2Regs[6]);
        DWLWriteReg(pDecContainer->dwl, 4 * 12, pDecContainer->mpeg2Regs[12]);

        DWLWriteReg(pDecContainer->dwl, 4 * 1, pDecContainer->mpeg2Regs[1]);
    }
                                ...
                                Mpeg2DecPicture * pPicture, u32 endOfStream)
{
    ...
    {
        (void) DWLmemset(pPicture, 0, sizeof(Mpeg2DecPicture));
                               ...
                               DecContainer * pDecCont, u32 picIndex)
{
    ...
    (void) DWLmemcpy(&pPicture->timeCode,
...
/*------------------------------------------------------------------------------
    ...
    Function name: Mpeg2DecMMUEnable

    Functional description:
        configure decoder to use buffers allocated through mmu 
...
------------------------------------------------------------------------------*/
Mpeg2DecRet Mpeg2DecMMUEnable(Mpeg2DecInst decInst, int mmu_enable)
{
    DecContainer *pDecCont = (DecContainer *) decInst;
	pDecCont->mmuEnable = mmu_enable;
