...
#ifndef IM_OMX_MACROS
#define IM_OMX_MACROS
...
#define INIT_SET_SIZE_VERSION(_struct_, _structType_)               \
    do {                                                            \
        IM_OSAL_Memset((_struct_), 0, sizeof(_structType_));       \
 ...
 * Port Specific
 */
#define IM_TUNNEL_ESTABLISHED 0x0001
#define IM_TUNNEL_IS_SUPPLIER 0x0002
...
#define CHECK_PORT_TUNNELED(port)                      (port->tunnelFlags & IM_TUNNEL_ESTABLISHED)
#define CHECK_PORT_BUFFER_SUPPLIER(port)               (port->tunnelFlags & IM_TUNNEL_IS_SUPPLIER)
#define SAFE_MALLOC(data, type, size) 	     \
	do{									     \
		data = (type *)IM_OSAL_Malloc(size); \
		if(data == NULL){					 \
			ret =  0;                          \
			goto EXIT;						 \
		}                                    \
