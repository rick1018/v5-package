...
#define DMX2ADEC2AO_DEFAULT_SRC_FILE        "/mnt/SDCARD/sample_demux2adec2ao/test.mp4"
...
ERRORTYPE MPPCallbackWrapper(void *cookie, MPP_CHN_S *pChn, MPP_EVENT_TYPE event, void *pEventData)
{
    ...
    else if (pChn->mModId == MOD_ID_ADEC)
    {
        ...
        case MPP_EVENT_NOTIFY_EOF:
            ...
            AW_MPI_AO_SetStreamEof(pSampleData->mAODevId, pSampleData->mAOChn, 1, 0);
            break;
        default:
            break;
        }
    ...
    else if (pChn->mModId == MOD_ID_AO)
    {
        ...
        case MPP_EVENT_NOTIFY_EOF:
            ...
            cdx_sem_signal(&pSampleData->mWaitEof);
            break;
        default:
            break;
        }
...
static ERRORTYPE createAOChn(SAMPLE_DEMUX2ADEC2AO_S *pSampleData)
{
    ...
    {
        ret = AW_MPI_AO_EnableChn(pSampleData->mAODevId, pSampleData->mAOChn);
        if(SUCCESS == ret)
        {
            bSuccessFlag = TRUE;
...
int main(int argc, char** argv)
{
    ...
    if (loadConfigPara(pDemux2Adec2AOData, pDemux2Adec2AOData->confFilePath) != SUCCESS)
    {
    ...
    cdx_sem_wait(&pDemux2Adec2AOData->mWaitEof);
    alogd("play finished!");
    ...
    {
        AW_MPI_AO_DisableChn(pDemux2Adec2AOData->mAOChn, pDemux2Adec2AOData->mAdecChn);
    }
   ...
   //exit mpp system
    ...
    printf("sample_demux2adec2ao exit!\n");
    return 0;
}
