...
int aw_vo_dev_creat(VO_Cap_S* pVoCap)
{
    ...
    spPubAttr.enIntfType = pVoCap->mDispType;
    spPubAttr.enIntfSync = pVoCap->mDispSync;
...
static ERRORTYPE loadSampleVirvi2Fish2VoConfig(SampleVirvi2Fish2VoConfig *pConfig, const char *conf_path)
{
    ...
    char *pStrDispType = (char*)GetConfParaString(&stConfParser, SAMPLE_Virvi2Fish2Vo_Display_Type, NULL);
    if (!strcmp(pStrDispType, "hdmi"))
    {
        pConfig->VOChnConfig.mDispType = VO_INTF_HDMI;
        if (pConfig->VOChnConfig.display_width > 1920)
            pConfig->VOChnConfig.mDispSync = VO_OUTPUT_3840x2160_30;
        else if (pConfig->VOChnConfig.display_width > 1280)
            pConfig->VOChnConfig.mDispSync = VO_OUTPUT_1080P30;
        else
            pConfig->VOChnConfig.mDispSync = VO_OUTPUT_720P60;
    }
    else if (!strcmp(pStrDispType, "lcd"))
    {
        pConfig->VOChnConfig.mDispType = VO_INTF_LCD;
        pConfig->VOChnConfig.mDispSync = VO_OUTPUT_NTSC;
    }
    else if (!strcmp(pStrDispType, "cvbs"))
    {
        pConfig->VOChnConfig.mDispType = VO_INTF_CVBS;
        pConfig->VOChnConfig.mDispSync = VO_OUTPUT_NTSC;
    }
...
int main(int argc, char *argv[])
{
    ...
    while (count != AutoTestCount)
    {
        ...
        pVOCap[0].mDispType = stContext.mConfigPara.VOChnConfig.mDispType;
        pVOCap[0].mDispSync = stContext.mConfigPara.VOChnConfig.mDispSync;
