...
#define DMX2VDEC2VO_DEFAULT_SRC_FILE   "/mnt/SDCARD/sample_demux2vdec2vo/test.mp4"
...
static ERRORTYPE configDmxChnAttr(SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData)
{
    ...
    pDemux2Vdec2VoData->mDmxChnAttr.mFd = pDemux2Vdec2VoData->mConfigPara.srcFd;
...
static ERRORTYPE createVoChn(SAMPLE_DEMUX2VDEC2VO_S *pDemux2Vdec2VoData)
{
   ...
   //enable vo layer
    ...
    {
        ret = AW_MPI_VO_EnableChn(pDemux2Vdec2VoData->mVoLayer, pDemux2Vdec2VoData->mVoChn);
        if (SUCCESS == ret)
        {
            nSuccessFlag = TRUE;
...
int main(int argc, char** argv)
{
    ...
    if (loadConfigPara(pDemux2Vdec2VoData, pDemux2Vdec2VoData->mCmdLinePara.mConfigFilePath) != SUCCESS)
    {
        aloge("no config file or parse conf file fail");
        goto err_out_0;
    }

    pDemux2Vdec2VoData->mConfigPara.srcFd = open(pDemux2Vdec2VoData->mConfigPara.srcFile, O_RDONLY);
    if (pDemux2Vdec2VoData->mConfigPara.srcFd < 0)
    {
...
err_out_2:
    ...
    if (pDemux2Vdec2VoData->mVoLayer >= 0)
    {
        if (pDemux2Vdec2VoData->mVoChn >= 0)
        {
            AW_MPI_VO_DisableChn(pDemux2Vdec2VoData->mVoLayer, pDemux2Vdec2VoData->mVoChn);
            pDemux2Vdec2VoData->mVoChn = MM_INVALID_CHN;
        }
...
err_out_1:
    ...
    close(pDemux2Vdec2VoData->mConfigPara.srcFd);
err_out_0:
    ...
    if (result == 0) {
        printf("sample_demux2vdec2vo exit!\n");
    }
