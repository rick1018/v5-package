...
#define DEFAULT_DST_VIDEO_FILE      "/mnt/SDCARD/sample_vi2venc2muxer/1080p.mp4"
#define DEFAULT_SRC_SIZE   1080
...
{
#if 0
    sprintf(pNameBuf, "%s", "/mnt/SDCARD/sample_mux/");
    ...
    else if(MOD_ID_MUX == pChn->mModId)
    {
        switch(event)
        {
            ...
            case MPP_EVENT_NEED_NEXT_FD:
            {
                ...
                {
                    strcpy(fileName, "/mnt/SDCARD/sample_vi2venc2muxer/");
                    static int cnt = 0;
                    cnt++;
...
#ifdef DOUBLE_ENCODER_FILE_OUT
    char mov_path[128];
    strcpy(mov_path, "/mnt/SDCARD/sample_vi2venc2muxer/0.ts");
...
/*
    ...
    AW_MPI_VENC_Set3DNR(pVi2Venc2MuxerData->mVeChn, FALSE);
