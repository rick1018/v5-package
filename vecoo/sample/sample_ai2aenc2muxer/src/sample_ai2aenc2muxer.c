...
#define DEFAULT_CONF_FILE_PATH      "/mnt/SDCARD/sample_ai2aenc2muxer/sample_ai2aenc2muxer.conf"
#define DEFAULT_DST_FILE_PATH       "/mnt/SDCARD/sample_ai2aenc2muxer/sample_ai2aenc2muxer.mp3"
#define DEFAULT_CAPTURE_DURATION    (20)   //Unit:Second
...
static int parseCmdLine(SAMPLE_AI2AENC2MUXER_S *pSampleData, int argc, char** argv)
{
    ...
    while (*argv)
    {
       ...
       {
            alogd("CmdLine param: -path /mnt/SDCARD/sample_ai2aenc2muxer/sample_ai2aenc2muxer.conf");
            break;
       }
       else if (*argv)
       {
...
static ERRORTYPE loadConfigPara(SAMPLE_AI2AENC2MUXER_S *pSampleData)
{
	...
	// Read config file
    FILE *fpConf = fopen(pSampleData->confFilePath, "rb");
    if (!fpConf)
    {// can't open config file, use default setting
        alogw("confFile [%s] NOT exist, use default conf param!", pSampleData->confFilePath);
    ...
    alogd("             cap_duration [%d], chn_cnt [%d], bit_width [%d], sample_rate [%d]",
        pSampleData->mConfCapDuration, pSampleData->mConfChnCnt, pSampleData->mConfBitWidth, pSampleData->mConfSampleRate);
...
void configAioAttr(SAMPLE_AI2AENC2MUXER_S *ctx)
{
    ...
    pAttr->enBitwidth   = (AUDIO_BIT_WIDTH_E)(ctx->mConfBitWidth / 8 - 1);
    pAttr->enSamplerate = (AUDIO_SAMPLE_RATE_E)ctx->mConfSampleRate;
}
...
int main(int argc, char** argv)
{
    ...
    strcpy(stSaveFileInfo.mFilePath, "/mnt/SDCARD/");
    strcpy(stSaveFileInfo.mFileName, "SampleAi2Aenc2Muxer_AiSaveFile.pcm");
    AW_MPI_AI_SaveFile(nSampleContext.mAIDevId, nSampleContext.mAIChnId, &stSaveFileInfo);
    ...
    while (1)
    {
        ...
        {// Get encodes frame sucess
            ...
            if (!(iCapPktCnt++ % 30))
            {
                ...
                AW_MPI_AI_QueryFileStatus(nSampleContext.mAIDevId, nSampleContext.mAIChnId, &stSaveFileInfo);
                alogd("AI comp save_file(%s%s) exist_flag:%d, current_file_size:%d",
                    stSaveFileInfo.mFilePath, stSaveFileInfo.mFileName, stSaveFileInfo.bCfg, stSaveFileInfo.mFileSize);
            }
...
_END:
    ...
    if (ret == 0) 
	{
	    printf("sample_ai2aenc2muxer exit!\n");
    }
    return ret;
}
