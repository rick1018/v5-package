...
static int media_enum_entities(struct media_device *media)
{
	...
	for (id = 0; ; id = entity->info.id) {
		...
		/* Number of links (for outbound links) plus number of pads (for * inbound links) is a good safe initial estimate of the total * number of links. */
		...
		if (entity->pads == NULL || entity->links == NULL)
			return -ENOMEM;
		media->entities_count++; /* Find the corresponding device name. */
		...
		if (ret < 0)
			continue;
		/* Sanity check: udev might have reordered the device nodes. * Make sure the major/minor match. We should really use * libudev. */
...
struct media_device *media_open(const char *name, int verbose)
{
	...
	if (ret < 0) {
		ISP_ERR("%s: Unable to retrieve media device information for " "device %s (%s)\n", __func__, name, strerror(errno));
		media_close(media);
		return NULL;
	}
	if (verbose)
	...
	if (verbose) {
		...
		for(i = 0; i < media->entities_count; i++)
			ISP_PRINT("Entities %d is %s!\n", i+1, media->entities[i].info.name);
...
void media_close(struct media_device *media)
{
	...
	for (i = 0; i < media->entities_count; ++i)
	{
		...
		free(entity->pads); free(entity->links);
		if (entity->fd != -1)
			close(entity->fd);
	}
	free(media->entities);
