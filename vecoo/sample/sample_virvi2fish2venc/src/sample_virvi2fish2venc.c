...
static ERRORTYPE loadSampleVirvi2Fish2VencConfig(SampleVirvi2Fish2VencConfig *pConfig, const char *conf_path)
{
    ...
    printf("VI Parameter:dev_id = %d, src_width = %d, src_height = %d, src_frame_rate = %d\n",
       pConfig->VIDevConfig.DevId,pConfig->VIDevConfig.SrcWidth,pConfig->VIDevConfig.SrcHeight,
    ...
    pConfig->ISEGroupConfig.ISE_Dewarp_Mode = GetConfParaInt(&stConfParser, SAMPLE_Virvi2Fish2Venc_ISE_Dewarp_Mode, 0);
    ...
    pConfig->ISEGroupConfig.Mount_Mode = GetConfParaInt(&stConfParser, SAMPLE_Virvi2Fish2Venc_ISE_Mount_Mode, 0);
...
{
    printf("Run CSI0/CSI1+ISE+Venc command: ./sample_virvi2fish2venc -path ./sample_virvi2fish2venc.conf\r\n");
}
...
int main(int argc, char *argv[])
{
    ...
    int result = 0;
    int AutoTestCount = 0;
    printf("sample_virvi2fish2venc build time = %s, %s.\r\n", __DATE__, __TIME__);
    if (argc != 3)
    {
    ...
    {
        pConfigFilePath = DEFAULT_SAMPLE_VIRVI2FISH2VENC_CONF_PATH;
    }
    //parse config file.
    ...
    while (count != AutoTestCount)
    {
        ...
        for(i = 0;i < VencChnNum;i++)
        {
            ...
            if(PT_H264 == pVENCCap[i].mVEncChnAttr.VeAttr.Type)
            {
                ...
                if(stContext.mConfigPara.VencChnConfig[i].mTimeLapseEnable)
                {
...
sys_exit:
    ...
    printf("sample_virvi2fish2venc exit!\n");
    return 0;
_exit:
        return result;
}
