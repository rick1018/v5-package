...
MPPCFG_BDII := N
#config if include libIRIDALABS_ViSta.so and enable video stabilization algorithm.
MPPCFG_VIDEOSTABILIZATION := N


#config if include libaudioaec.so and enable aec audio effect handle.
MPPCFG_AUDIO_EFFECT_AEC := N
#config if include libaudioeq.so and enable eq audio effect handle.
MPPCFG_AUDIO_EFFECT_EQ := N
#config if include libAudioGain.so and enable gain audio effect handle.
MPPCFG_AUDIO_EFFECT_GAIN := N
#config if include libaudioresample.so and enable resample audio effect handle.
MPPCFG_AUDIO_EFFECT_RESAMPLE := N
#config if include libAwDrc.so and enable drc audio effect handle.
MPPCFG_AUDIO_EFFECT_DRC := N
#config if include libAwNosc.so and enable rnr audio effect handle.
MPPCFG_AUDIO_EFFECT_RNR := N
...
ifeq ($(MPPCFG_MOD),Y)
  CEDARX_EXT_CFLAGS += -DMPPCFG_MOD=1#OPTION_MOD_ENABLE
else
  CEDARX_EXT_CFLAGS += -DMPPCFG_MOD=0#OPTION_MOD_DISABLE
endif
...
ifeq ($(MPPCFG_VIDEOSTABILIZATION),Y)
  CEDARX_EXT_CFLAGS += -DMPPCFG_VIDEOSTABILIZATION=1#OPTION_VIDEOSTABILIZATION_ENABLE
else
  CEDARX_EXT_CFLAGS += -DMPPCFG_VIDEOSTABILIZATION=0#OPTION_VIDEOSTABILIZATION_DISABLE
endif


ifeq ($(MPPCFG_AUDIO_EFFECT_AEC),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_AEC
endif

ifeq ($(MPPCFG_AUDIO_EFFECT_EQ),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_EQ
endif

ifeq ($(MPPCFG_AUDIO_EFFECT_GAIN),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_GAIN
endif

ifeq ($(MPPCFG_AUDIO_EFFECT_RESAMPLE),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_RESAMPLE
endif

ifeq ($(MPPCFG_AUDIO_EFFECT_DRC),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_DRC
endif

ifeq ($(MPPCFG_AUDIO_EFFECT_RNR),Y)
  CEDARX_EXT_CFLAGS += -DCFG_AUDIO_EFFECT_RNR
endif

ifeq ($(MPPCFG_USE_KFC),Y)
  CEDARX_EXT_CFLAGS += -DMPPCFG_USE_KFC
endif
