...
/************************************************************************************************/
/**
 * @file mpp_vi_venc_mux.cpp
 * @brief 该目录是对mpp中的 VI+VENC+MUX 通路sample
...
static MENU_INODE g_menu_top[] = {
    ...
    {(char *)"Quite vi+(venc+mux) sample",  ExitCurrentMenuLevel, NULL, NULL},
    {NULL, NULL, NULL, NULL},
};
...
#endif
{
    ...
    printf("\033[2J");
    printf("\n\n\nDo sample vi+venc+mux. default:VI_4K@30fps+VENC(4K@30fps+VGA@30fps) H264/CBR/MainProfile\n");
    ...
    /* Setup 4. Create MUX Group and Channel */
    ret = mux_create();
    if (ret < 0)
    {
    ...
    /* Setup 5. Bind vi_0 to venc_0  and vi_1 to venc_1 */
    ret = components_bind();
    if (ret < 0)
    {
