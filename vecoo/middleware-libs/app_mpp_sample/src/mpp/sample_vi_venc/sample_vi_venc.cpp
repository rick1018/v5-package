...
static int vi_create(void)
{
    ...
    ret = mpp_comm_vi_dev_create(VI_DEV_2, &vi_attr);
    if (ret)
    {
    ...
    ret = mpp_comm_vi_chn_create(VI_DEV_2, VI_CHN_0);
    if (ret)
    {
        ERR_PRT("Do mpp_comm_vi_chn_create fail! ret:%d  vi_dev:%d  vi_chn:%d \n", ret, VI_DEV_2, VI_CHN_0);
        return -1;
    }
...
static int vi_destroy(void)
{
    ...
    /**  destroy vi dev 2  **/
    ret = mpp_comm_vi_chn_destroy(VI_DEV_2, VI_CHN_0);
    if (ret)
    {
        ERR_PRT("Do mpp_comm_vi_chn_destroy fail! ret:%d  vi_dev:%d  vi_chn:%d \n", ret, VI_DEV_2, VI_CHN_0);
    }
    ret = mpp_comm_vi_dev_destroy(VI_DEV_2);
    if (ret)
    {
...
static int components_bind(void)
{
    ...
    ret = mpp_comm_vi_bind_venc(VI_DEV_2, VI_CHN_0, VENC_CHN_0);
    if (ret)
    {
...
static int components_unbind(void)
{
    ...
    ret = mpp_comm_vi_unbind_venc(VI_DEV_2, VI_CHN_0, VENC_CHN_0);
    if (ret)
    {
...
static int components_start(void)
{
    ...
    ret = AW_MPI_VI_EnableVirChn(VI_DEV_2, VI_CHN_0);
    if (ret)
    {
...
static int components_stop(void)
{
    ...
    ret = AW_MPI_VI_DisableVirChn(VI_DEV_2, VI_CHN_0);
    if (ret)
    {
...
#endif
{
    ...
    int d_val;
    while (1) {
	    AW_MPI_ISP_GetIspDigGain(0, &d_val); 
	    printf("FGGGGGGGGGGGGG: %d\n", d_val);
	    sleep(1);
    }
