...
int start_video(capture_params *cap_pa, capture_format *cap_fmt)
{
    ...
    if(vi_venc_rtsp_en)
    {
    	if (g_sensor_set_flag & (1<<cap_fmt->channel)) {
    		LOG("%s: ch%d is for RTSP, please select other channels\n", __FUNCTION__, cap_fmt->channel);
    		return CAP_ERR_INVALID_PARAMS;
    	}
    }   
#endif
...
int set_sensor_input(const sensor_input *sensor_in)
{
	...
	if (sensor_in && VALID_VIDEO_SEL(sensor_in->channel)) {
        ...
        if(vi_venc_rtsp_en)
        {
    		start_venc_rtsp_module(sensor_in);
    		cap_handle->status |= CAP_STATUS_ON;
    		cap_handle->status |= CAP_THREAD_RUNNING;
...
#ifdef SUPPORT_VENC_RTSP
void start_venc_rtsp_module(const sensor_input *sensor_in)
{
    int ret = 0;
    Virvi2VencConfig Config;

    memset(&Config, 0, sizeof(Virvi2VencConfig));
    strncpy(Config.OutputFilePath, "test_out.h264", MAX_FILE_PATH_SIZE-1);
    Config.OutputFilePath[MAX_FILE_PATH_SIZE-1] = '\0';
    Config.DevNum = sensor_in->channel;
    //Config.DevNum = 1;
    Config.SrcFrameRate = sensor_in->fps;
    Config.SrcWidth = sensor_in->width;
    Config.SrcHeight = sensor_in->height;
    Config.DestPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
    Config.EncoderType = PT_H264;
    Config.DestWidth = sensor_in->width;
    Config.DestHeight = sensor_in->height;
    Config.DestFrameRate = sensor_in->fps;
    Config.DestBitRate = 4 * (1 << 20);
    LOG("%s: dev_num=%d, src_width=%d, src_height=%d, src_frame_rate=%d\n", __FUNCTION__,
            Config.DevNum, Config.SrcWidth,
            Config.SrcHeight, Config.SrcFrameRate);
    LOG("%s:dest_width=%d, dest_height=%d, dest_frame_rate=%d, dest_bit_rate=%d\n", __FUNCTION__,
            Config.DestWidth, Config.DestHeight,
            Config.SrcFrameRate, Config.DestBitRate);

    ret = init_venc_module(&Config);
    if (ret < 0) {
        LOG("%s: init venc module failed!\n", __FUNCTION__);
    } else {
        LOG("%s: init venc module OK!\n", __FUNCTION__);
    }
}

void stop_venc_rtsp_module()
{
    int ret = deinit_venc_module();
    if (ret < 0) {
        LOG("%s: deinit venc module failed!\n", __FUNCTION__);
    } else {
        LOG("%s: deinit venc module OK!\n", __FUNCTION__);
    }
}
#endif
