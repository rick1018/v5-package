	...
	convert_long_func cvt_long, convert_short_func cvt_short)
{
	...
	case HW_ISP_CFG_DYNAMIC:
		...
		break;
	case HW_VENC_CFG_COMMON:
		if (cfg_ids & HW_VENC_CFG_COMMON_3DNR){
			DD("venc_3DNR: ");
			int_ptr = (HW_S32 *)cfg_data;
			attr_len = sizeof( struct venc_3DNR_cfg) / sizeof(HW_S32);
			for (i = 0; i < attr_len; i++, int_ptr++) {
				*int_ptr = cvt_long(*int_ptr);
			}
			DD("\n");
			/* offset */
			cfg_data += sizeof(struct venc_3DNR_cfg);
		}

		if (cfg_ids & HW_VENC_CFG_COMMON_PROC){
			DD("venc_PROC: ");
			int_ptr = (HW_S32 *)cfg_data;
			attr_len = sizeof(struct venc_proc_cfg) / sizeof(HW_S32);
			for (i = 0; i < attr_len; i++, int_ptr++) {
				*int_ptr = cvt_long(*int_ptr);
			}
			DD("\n");
			/* offset */
			cfg_data += sizeof(struct venc_proc_cfg);
		}

		if (cfg_ids & HW_VENC_CFG_COMMON_SAVEBSFILE){
			DD("venc_SAVEBSFILE: ");
#if DO_LOG_EN
			int_ptr = (HW_S32 *)cfg_data;
			attr_len = sizeof(struct venc_savebsfile_cfg) / sizeof(HW_S32);
			for (i = 0; i < attr_len; i++, int_ptr++) {
				*int_ptr = cvt_long(*int_ptr);
			}
#endif
			...
			DD("\n");
			/* offset */
			cfg_data += sizeof(struct venc_savebsfile_cfg);
		}
		break;

	case HW_VENC_CFG_H264:
		if (cfg_ids & HW_VENC_CFG_H264_ATTR_CBR ||cfg_ids & HW_VENC_CFG_H264_ATTR_VBR||cfg_ids & HW_VENC_CFG_H264_ATTR_FIXQP || cfg_ids & HW_VENC_CFG_H264_ATTR_ABR){
			
			DD("venc_ATTR: ");
			int_ptr = (HW_S32 *)cfg_data;
			attr_len = sizeof(struct venc_attr_H264_H265_cfg) / sizeof(HW_S32);
			for (i = 0; i < attr_len; i++, int_ptr++) {
				*int_ptr = cvt_long(*int_ptr);
			}
			DD("\n");
			/* offset */
			cfg_data += sizeof(struct venc_attr_H264_H265_cfg);
		}

		if (cfg_ids & HW_VENC_CFG_H264_RC_CBR ||cfg_ids & HW_VENC_CFG_H264_RC_VBR){
			
			DD("venc_RC: ");
			int_ptr = (HW_S32 *)cfg_data;
			attr_len = sizeof( struct venc_rcparam_H264_265_cfg) / sizeof(HW_S32);
			for (i = 0; i < attr_len; i++, int_ptr++) {
				*int_ptr = cvt_long(*int_ptr);
			}
			DD("\n");
			/* offset */
			cfg_data += sizeof( struct venc_rcparam_H264_265_cfg);
		}
		break;
	case HW_VENC_CFG_H265:
		if (cfg_ids & HW_VENC_CFG_H265_ATTR_CBR ||cfg_ids & HW_VENC_CFG_H265_ATTR_VBR||cfg_ids & HW_VENC_CFG_H265_ATTR_FIXQP || cfg_ids & HW_VENC_CFG_H265_ATTR_ABR){
		...
		if (cfg_ids & HW_VENC_CFG_H265_RC_CBR ||cfg_ids & HW_VENC_CFG_H265_RC_VBR){
		...
		if (SOCK_CMD_STAT_AE == type) {
			...
			for (i = 0; i < ISP_AE_ROW; i++) {
				for (j = 0; j < ISP_AE_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", ae_info->accum_r[i][j]);
				}
			...
			for (i = 0; i < ISP_AE_ROW; i++) {
				for (j = 0; j < ISP_AE_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", ae_info->accum_g[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// accum_b
			...
			for (i = 0; i < ISP_AE_ROW; i++) {
				for (j = 0; j < ISP_AE_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", ae_info->accum_b[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// avg_max
			index += sprintf(&buf[index], "-------------- avg_max ----------------\n");
			for (i = 0; i < ISP_AE_ROW; i++) {
				for (j = 0; j < ISP_AE_COL; j++) {
					index += sprintf(&buf[index], "%06u, ", ae_info->avg_max[i * ISP_AE_COL + j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// avg
			...
			for (i = 0; i < ISP_AWB_ROW; i++) {
				for (j = 0; j < ISP_AWB_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", awb_info->awb_sum_r[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// awb_sum_g
			...
			for (i = 0; i < ISP_AWB_ROW; i++) {
				for (j = 0; j < ISP_AWB_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", awb_info->awb_sum_g[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// awb_sum_b
			...
			for (i = 0; i < ISP_AWB_ROW; i++) {
				for (j = 0; j < ISP_AWB_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", awb_info->awb_sum_b[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// awb_sum_cnt
			...
			for (i = 0; i < ISP_AWB_ROW; i++) {
				for (j = 0; j < ISP_AWB_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", awb_info->awb_sum_cnt[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// awb_avg_r
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", af_info->af_count[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_h_d1
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", af_info->af_h_d1[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_h_d2
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", af_info->af_h_d2[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_v_d1
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", af_info->af_v_d1[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
			// af_v_d2
			...
			for (i = 0; i < ISP_AF_ROW; i++) {
				for (j = 0; j < ISP_AF_COL; j++) {
					index += sprintf(&buf[index], "%06llu, ", af_info->af_v_d2[i][j]);
				}
				index += sprintf(&buf[index], "\n");
			}
...
void hton_3a_info(void *stat_info, int type)
{
	if (stat_info) {
		...
		if (SOCK_CMD_STAT_AE == type) {
			...
			// accum_r
			ptr_u64 = &ae_stat->accum_r[0][0];
			for (i = 0; i < ISP_AE_ROW * ISP_AE_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// accum_g
			ptr_u64 = &ae_stat->accum_g[0][0];
			for (i = 0; i < ISP_AE_ROW * ISP_AE_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// accum_b
			ptr_u64 = &ae_stat->accum_b[0][0];
			for (i = 0; i < ISP_AE_ROW * ISP_AE_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// avg_max
			ptr_u32 = &ae_stat->avg_max[0];
			...
			// awb_sum_r
			ptr_u64 = &awb_stat->awb_sum_r[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_sum_g
			ptr_u64 = &awb_stat->awb_sum_g[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_sum_b
			ptr_u64 = &awb_stat->awb_sum_b[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_sum_cnt
			ptr_u64 = &awb_stat->awb_sum_cnt[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_avg_r
			ptr_u64 = (HW_U64 *)&awb_stat->awb_avg_r[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_avg_g
			ptr_u64 = (HW_U64 *)&awb_stat->awb_avg_g[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
			// awb_avg_b
			ptr_u64 = (HW_U64 *)&awb_stat->awb_avg_b[0][0];
			for (i = 0; i < ISP_AWB_ROW * ISP_AWB_COL; i++, ptr_u64++) {
				*ptr_u64 = htonl(*ptr_u64);
			}
		} else if (SOCK_CMD_STAT_AF == type) {
