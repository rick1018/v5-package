...
GopRet_t LockGopBySerial(int serial, VIDEO_BLK_INFO *pVidInfo, int index)
{
	...
	Sem_lock(STREAM_SEM_GOP);//RS,0409+
	if ((pGopInfo = GetGopInfoBySerial(serial, index)) != NULL)
	{
//		Sem_lock(STREAM_SEM_GOP);//RS,0409-
		pGopInfo->lock++;
//		Sem_unlock(STREAM_SEM_GOP);//RS,0409-
	}
	else
	{
...
GopRet_t AddNewGop(int start_serial, int end_serial, int index)
{
	GopInfo_t *NewGop = GopInfoMalloc();
	if (NewGop == NULL)
		return GOP_NOMEM;
...
int GetGopLastPwithStartSerial(int start_serial, VIDEO_BLK_INFO *pVidInfo)
{
	int LastP;
	VIDEO_FRAME* pFrame;
	for (LastP = start_serial + 1;(pFrame =	MemMng_GetFrameBySerial(LastP, pVidInfo)) != NULL;LastP++)
    {
		if(pFrame->fram_type == I_FRAME)
			return LastP-1;
    }
...
//	GopInfo_t* pGopInfo = GetGopInfoBySerial(cur_serial - 1,index);//RS,0409-
	GopInfo_t* pGopInfo;	//RS,0409+

	Sem_lock(STREAM_SEM_GOP);//RS,0409+
	pGopInfo = GetGopInfoBySerial(cur_serial - 1,index);//RS,0409+
	if (pGopInfo == NULL)
	{
		ERR("cur_serial = %d\n",cur_serial);
		ShowAllGop(index);
