...
CACHE_MNG	Cache_mng_default[VIDEO_INFO_END] =
{
	...
	},
	{
		...
		.start_addr			= 0,
		.start_phy			= 0,
		.size				= MP4_3_CACHE_SIZE,
		.blk_sz				= MP4_3_CACHE_BLK_SIZE,
		.blk_num			= (MP4_3_CACHE_SIZE/MP4_3_CACHE_BLK_SIZE),
		.cache_num			= (MP4_3_CACHE_SIZE/MP4_3_CACHE_BLK_SIZE),
		.cache 				= NULL,
		.blk 				= NULL,
	},	
	{
	...
	},
	{
		...
		.start_addr			= 0,
		.start_phy			= 0,
		.size				= AUDIO_2_CACHE_SIZE,
		.blk_sz				= AUDIO_2_CACHE_BLK_SIZE,
		.blk_num			= (AUDIO_2_CACHE_SIZE/AUDIO_2_CACHE_BLK_SIZE),
		.cache_num			= (AUDIO_2_CACHE_SIZE/AUDIO_2_CACHE_BLK_SIZE),
		.cache 				= NULL,
		.blk 				= NULL,
	}
};
...
//		return CACHE_PARM_ERR;
	...
	unsigned char *avframedata = pParm->MemInfo.start_phyAddr;
	__D("CacheMng_memcpy 0x%x 0x%x\n",(unsigned long)Des,(unsigned long)Src);
//       CacheMng_printheader(avframedata + (unsigned long)Src,5); 
	memcpy( (void*)avframedata + (unsigned long)Des, (void*)avframedata + (unsigned long)Src , size );
//       CacheMng_printheader(avframedata + (unsigned long)Des,5); 
#endif
...
int CacheMng_Init(MEM_MNG_INFO *pInfo)
{
	...
	for (cnt = 0; cnt < loops ; cnt++)
	{
		if (Cache_mng_default[cnt].video_type == pInfo->video_info[cnt].video_type)
		{
			...
			pInfo->video_info[cnt].cachemng->cache	= calloc(pInfo->video_info[cnt].cachemng->cache_num, sizeof(CACHE_DATA));
			pInfo->video_info[cnt].cachemng->blk = calloc(pInfo->video_info[cnt].cachemng->blk_num,	sizeof(CACHE_BLK));
			...
			{
				CacheMng_blk_reset(&(pInfo->video_info[cnt].cachemng->blk[cnt2]), (cnt2*pInfo->video_info[cnt].cachemng->blk_sz));
			}
...
void CacheMng_printheader(char *pData, int size)
{
	int cnt = 0;
	...
	for (cnt = 0 ;cnt < size; cnt++ )
	{
		...
		if (cnt != 0 && cnt % 4 == 0)  fprintf(stderr,"\n");
	}
}
...
//       ShowLockInfo();
	...
	{	
		...
		__D("video_type = %d, pFrame = %p %d %d %d %d\n",pVidInfo->video_type, pFrame, blk_sz, pVidInfo->cachemng->blk_sz,pVidInfo->cur_serial,serial);		
		blk_index	 = CacheMng_Find_Freeblk( pVidInfo , blk_use );
		if (blk_index < 0)
		{
			__D("pFrame->realsize = %d \n",pFrame->realsize);
			__D("CacheMng_Video_Cache: CacheMng_Find_Freeblk Fail \n");
			ret = CACHE_NOMEM;
			goto CACHE_FAIL;
		}
		...
		if (cache_index < 0)
		{
			__D("CacheMng_Video_Cache: CacheMng_Find_FreeCache Fail \n");
			ret = CACHE_NOMEM;
			goto CACHE_FAIL;
		}
...
CACHE_FAIL:
	__D("CacheMng_Video_Cache: CACHE_FAIL \n");
	return ret;
}
...
int CacheMng_Release_ch(MEM_MNG_INFO *pInfo, int ch)
{
	int	cnt = ch;
	int cnt2 = 0;
	fprintf(stderr,"CacheMng_Release_ch0...\n");
	...
	//for (cnt = 0; cnt < pInfo->video_info_nums; cnt++)
	{
		if (pInfo->video_info[cnt].IsCache )
		{
			if (pInfo->video_info[cnt].cachemng->cache)
			{
				...
				for (cnt2 =0 ; cnt2 < pInfo->video_info[cnt].cachemng->cache_num ; cnt2++)
				{
				//fprintf(stderr,"CacheMng_Release_ch1...\n");
					//CACHE_DATA *pcache = &(pInfo->video_info[cnt].cachemng->cache[cnt2]);
					CacheMng_cache_reset(&(pInfo->video_info[cnt].cachemng->cache[cnt2]));
					//fprintf(stderr,"CacheMng_Release_ch3...\n");
				}
			}
			...
			if (pInfo->video_info[cnt].cachemng->blk)
			{
				...
				for (cnt2 =0 ; cnt2 < pInfo->video_info[cnt].cachemng->blk_num ; cnt2++)
				{
					//CacheMng_blk_unset(&(pInfo->video_info[cnt].cachemng->blk[cnt2]));//, (cnt2*pInfo->video_info[cnt].cachemng->blk_sz));
					CacheMng_blk_reset(&(pInfo->video_info[cnt].cachemng->blk[cnt2]), (cnt2*pInfo->video_info[cnt].cachemng->blk_sz));
				}				
			}
