...
{
   int s;
   s = ((AVI->track[j].a_bits+7)>>3)*AVI->track[j].a_chans;//s = ((AVI->track[j].a_bits+7)/8)*AVI->track[j].a_chans;
...
static int avi_add_index_entry(avi_t *AVI, unsigned char *tag, long flags, unsigned long pos, unsigned long len)
{
   ...
   if(AVI->n_idx>=AVI->max_idx) {
     ptr = realloc((void *)AVI->idx,(AVI->max_idx+4096)<<4);//ptr = realloc((void *)AVI->idx,(AVI->max_idx+4096)*16);
...
int avi_update_header(avi_t *AVI)
{
   ...
   if(AVI->fps < 0.001) {
     ...
     ms_per_frame=(int) (1000000.0/AVI->fps + 0.5);
   }
...
//   OUTLONG(64);                 /* # of bytes to follow */
   ...
   OUTLONG(frate);              /* Rate: Rate/Scale == samples/second  = frame rate */
   OUTLONG(0);                  /* Start */
   OUTLONG(0);                  // no frames yet
   OUTLONG(0);                  /* SuggestedBufferSize */
   OUTLONG(-1);                 /* Quality */
...
//       OUTLONG(64);            /* # of bytes to follow */
       ...
       // ThOe /4
       //OUTLONG(sampsize>>2);//OUTLONG(sampsize/4);      /* Scale */
//       OUTLONG(4);//by frank fo PCM
       OUTLONG(sampsize);//by frank fo PCM
//       OUTLONG((1000*AVI->track[j].mp3rate)>>3);//OUTLONG(1000*AVI->track[j].mp3rate/8);
//       OUTLONG(4*16000);////by frank fo PCM
       OUTLONG(sampsize*AVI->track[j].a_rate); /* Rate: Rate/Scale == samples/second */
       OUTLONG(0);             /* Start */
//       OUTLONG(4*AVI->track[j].audio_bytes/sampsize);   /* Length */
       OUTLONG(AVI->track[j].audio_bytes/sampsize);   /* Length */	   
       OUTLONG(0);             /* SuggestedBufferSize */
       OUTLONG(-1);            /* Quality */

       // ThOe /4
      // OUTLONG(sampsize>>2);//OUTLONG(sampsize/4);    /* SampleSize */
      OUTLONG(4);////by frank fo PCM
...
//       OUTLONG((1000*AVI->track[j].mp3rate)>>3);//OUTLONG(1000*AVI->track[j].mp3rate/8);
	   ...
	   OUTLONG(sampsize*AVI->track[j].a_rate); /* AvgBytesPerSec */ 
	   OUTSHRT(sampsize);             /* BlockAlign */   
       //ThOe (/4)
//       OUTSHRT(sampsize>>2);//OUTSHRT(sampsize/4);           /* BlockAlign */
//	   OUTSHRT(4);//by frank fo PCM		 /* BlockAlign */
   ...
   njunk = HEADERBYTES - nhb - 20;//njunk = HEADERBYTES - nhb - 8 - 12;
...
//   time_t calptr;
   ...
   {  // add by kenny chiu ti fix real fps to avi file 20130221
   	//double dSec = (AVI->track[AVI->aptr].audio_bytes / 8000.0);
    double dSec = 0;
    double dFps = AVI->fps;

	if (AVI->track[AVI->aptr].audio_bytes > 0)
	{
	  dSec = (AVI->track[AVI->aptr].audio_bytes / (AVI->track[AVI->aptr].a_chans * AVI->track[AVI->aptr].a_rate * (AVI->track[AVI->aptr].a_bits / 8)));   	
 	  if (dFps > (AVI->video_frames / dSec)) AVI->fps = AVI->video_frames / dSec;
	}
	
// 	fprintf(stderr, "audio_bytes1 = %d, total sec = %f, total frame = %d, ori fps = %f, new fps = %f\n",AVI->track[AVI->aptr].audio_bytes, dSec, AVI->video_frames, dFps, AVI->fps);
   }
#endif
...
//         OUTLONG(sampsize>>2);//OUTLONG(sampsize/4);      /* Scale */
//#else
//         OUTLONG(4);//by frank for PCM      /* Scale */
         OUTLONG(sampsize);//by frank for PCM      /* Scale */
//#endif
//#ifndef GC_6500

//         OUTLONG((1000*AVI->track[j].mp3rate)>>3);//OUTLONG(1000*AVI->track[j].mp3rate/8);
//#else         
//         OUTLONG(16000*4);//by frank for PCM 16bit 1600rate, stero
         OUTLONG(sampsize*AVI->track[j].a_rate);//by frank for PCM 16bit 1600rate, stero
//#endif
         OUTLONG(0);             /* Start */
//#ifndef GC_6500
//         OUTLONG(4*AVI->track[j].audio_bytes/sampsize);   /* Length */
//#else
//		 OUTLONG(4*AVI->track[j].audio_bytes/16);   /* Length */
         OUTLONG(AVI->track[j].audio_bytes/sampsize);   /* Length */
//#endif
		 //OUTLONG(4);   /* Length */
         OUTLONG(0);             /* SuggestedBufferSize */
...
//#else
//         OUTLONG(16000*4);//OUTLONG(1000*AVI->track[j].mp3rate/8);
         OUTLONG(sampsize*AVI->track[j].a_rate); /* AvgBytesPerSec */
//#endif
         //ThOe (/4)
//#ifndef GC_6500          
//         OUTSHRT(sampsize>>2);//OUTSHRT(sampsize/4);           /* BlockAlign */
//#else
//         OUTSHRT(4);//OUTSHRT(sampsize/4);           /* BlockAlign */
         OUTSHRT(sampsize);             /* BlockAlign */
//#endif         
         OUTSHRT(AVI->track[j].a_bits);          /* BitsPerSample */
...
static int avi_write_data(avi_t *AVI, unsigned char *data, unsigned long length, int audio, int keyframe)
{
   ...
   if ( (AVI->pos + length + 16 + ((AVI->n_idx+1)<<4)) > AVI_MAX_LEN ) {//if ( (AVI->pos + 8 + length + 8 + (AVI->n_idx+1)*16) > AVI_MAX_LEN ) {
...
int AVI_append_audio(avi_t *AVI, char *data, long bytes)
{
  ...
  ++AVI->n_idx;
	...
	long AVI_bytes_remain(avi_t *AVI)
	{
	   ...
	   return ( AVI_MAX_LEN - (AVI->pos + 8 + (AVI->n_idx<<4)));//return ( AVI_MAX_LEN - (AVI->pos + 8 + 16*AVI->n_idx));
	}
	...
	long AVI_bytes_written(avi_t *AVI)
	{
	   ...
	   return (AVI->pos + 8 + (AVI->n_idx<<4));//return (AVI->pos + 8 + 16*AVI->n_idx);
	}
	   ...
	   while(1)
	   {
	      ...
	      if(strncasecmp(data,"LIST",4) == 0)
	      {
	         ...
	         if(strncasecmp(data,"hdrl",4) == 0)
	         {
	            ...
	            if(hdrl_data==NULL) ERR_EXIT(AVI_ERR_NO_MEM);
	      ...
	      else if(strncasecmp(data,"idx1",4) == 0)
	      {
	         ...
	         AVI->n_idx = AVI->max_idx = n>>4;//n/16;

	         AVI->idx = NULL;
	         AVI->idx = (unsigned  char((*)[16]) ) malloc(n);
	         if(AVI->idx==NULL) ERR_EXIT(AVI_ERR_NO_MEM)
	   ...
	   for(i=0;i<hdrl_len;)
	   {
	      ...
	      else if(strncasecmp((char *)hdrl_data+i,"strf",4)==0)
	      {
	         ...
	         else if(lasttag == 2)
	         {
	            ...
	            if (wfe != NULL) {
	              ...
	              if (wfe->cb_size != 0) {
	                ...
	                if (nwfe != 0) {
	                  ...
	                  lseek(AVI->fdes, header_offset + i + sizeof(WAVEFORMATEX_avilib), SEEK_SET);
	            ...
	            AVI->track[AVI->aptr].mp3rate = (str2ulong(hdrl_data+i+8)<<3)/1000;//AVI->track[AVI->aptr].mp3rate = 8*str2ulong(hdrl_data+i+8)/1000;
	            //:ThOe
