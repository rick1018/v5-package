...
/* 20140225 */
void SendAlarmTrigger(int index,Event_t event,int alarmaudio_duration, int alarm_audio_enable)
{
	ALARM_MSG_BUF msgbuf;
	memset(&msgbuf, 0, sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	...
	DBG("%s %d %d %d %d\n",__func__, event, index, alarmaudio_duration, alarm_audio_enable);
