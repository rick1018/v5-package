...
#define TRACE(arg, ...)  printf("TRACE:"arg, ##__VA_ARGS__);
#else
#define TRACE(arg, fmt...)
#endif
...
int gpio_dev_open(void)
{
	TRACE("Opening GPIO device\n");
	int fd = open(GPIO_DEV, O_RDONLY);
	if (fd < 0) { perror(GPIO_DEV); return -1; }
//  ioctl(fd, RALINK_GPIO3924_SET_DIR_OUT, (1 << (GPIO_LED_AMBER - 24)));
//  ioctl(fd, RALINK_GPIO72_SET_DIR_OUT,  (1<< (72-GPIO_LED_GREEN)));
	TRACE("gpio_dev_open fd %d\n",fd);
	return fd;
}

void gpio_dev_close(int fd)
{        
	TRACE("Closing GPIO device\n");
	close(fd);
}
/* @brief read mt7620 gio infomation
*
*@param gio_id GIO ID
*@return GIO status
*@retval -1  error
...
int dm355_gio_read(int gio_id)
{
	...
	int d;
	FILE *fp = NULL;
	if ((fp = popen("cat /proc/gpio", "r")) == NULL) return 1;
	fscanf(fp, "Reset %d", &d);
	pclose(fp);
	//   printf("reset =%d\n",d);
	return (1 - d);
    ...
    fd=gpio_dev_open();    
#if 0    
    if (gio_id < 24) {  ioctl(fd, RALINK_GPIO_READ, &data); data = ((data >> gio_id) & 1);	} 
  	else if (gio_id < 40){	ioctl(fd, RALINK_GPIO3924_READ, &data);	data = ((data >> (gio_id - 24)) & 1);	}	
	  else if (gio_id < 72){	ioctl(fd, RALINK_GPIO7140_READ, &data);	data = ((data >> (gio_id - 40)) & 1);	}	
    else { ioctl(fd, RALINK_GPIO72_READ, &data); data = ((data >> (gio_id - 72)) & 1);	}	
#else
   if (gio_id < 32) {  ioctl(fd, RALINK_GPIO_READ, &data); data = ((data >> gio_id) & 1);	}   
   else if (gio_id < 64) {	ioctl(fd, RALINK_GPIO6332_READ, &data);	data = ((data >> gio_id) & 1);	}	
   else if (gio_id < 96) {	ioctl(fd, RALINK_GPIO9564_READ, &data);	data = ((data >> gio_id) & 1);	}	 
#endif    
//    fprintf(stderr,"gpio %d = 0x%x\n", gio_id,data);
...
//#define	IR_CTRL			3
//#define	WHITE_LED		4
//#define	USB_POWER_CTRL		5
//#define	CAMEAR_POWER_CTRL	6
//#define	USB_HUB_CTRL		7
/* @brief set mt7620 gio infomation
*
*@param gio_id GIO ID
...
//   FILE *fp = NULL;
	...
	if (gio_id == GIO_PWR_LED)
		sprintf(cmd, "echo \"%d %d\" >> /proc/gpio", POWER_LED, IsOn);
	else if (gio_id == GIO_LAN_LED)
		sprintf(cmd, "echo \"%d %d\" >> /proc/gpio", LAN_LED, IsOn);
	else if (gio_id == GIO_ACT_LED)
		sprintf(cmd, "echo \"%d %d\" >> /proc/gpio", ACT_LED, IsOn);

//   fprintf(stderr,"%s[%d] gio_id=%d, val=%d\n",__FILE__,__LINE__,gio_id,IsOn);
//   fprintf(stderr,"%s\n",cmd);
   system(cmd);

//   if ((fp = popen(cmd, "w")) == NULL) return 1;
//   pclose(fp);
  ...
  if(gio_id <24)
  {
	req = (IsOn == LED_ON) ? RALINK_GPIO_CLEAR : RALINK_GPIO_SET;
	val = 1 << (gio_id); 
  }		
  else if(gio_id < 40)
  {
	...
	tmp = *(volatile unsigned *)(RALINK_REG_PIO7140DATA);
	if(IsOn == LED_ON) tmp &= (~(RALINK_GPIO(gio_id-40))); 
	else tmp |= RALINK_GPIO(gio_id-40);
	*(volatile unsigned  *)(RALINK_REG_PIO7140DATA) = tmp;
//	req = (IsOn == LED_ON) ? RALINK_GPIO3924_CLEAR : RALINK_GPIO3924_SET;
//	gpio_read_int(gpio7140, &d);
//    gpio_write_int(gpio7140,d);
//    req = RALINK_GPIO7140_WRITE;
//	val = (1 << (gio_id - 24)) & d; 
    fprintf(stderr,"%s[%d] gio_id=%d, val=%d,%d\n",__FILE__,__LINE__,gio_id,IsOn,d);
  }
  else if(gio_id < 72)
  {
	req = (IsOn == LED_ON) ? RALINK_GPIO7140_CLEAR : RALINK_GPIO7140_SET;
	val = 1 << (gio_id - 40);
  }
  else if(gio_id == 72)  	
  {
	req = (IsOn == LED_ON) ? RALINK_GPIO72_CLEAR : RALINK_GPIO72_SET;
	val = 1 << (gio_id - 72);
  }	

//  fd=gpio_dev_open();
//  if(ioctl(fd, req, val) < 0) { fprintf(stderr,"%s[%d] ioctl error",__FILE__,__LINE__); }
//  gpio_dev_close(fd);
#endif 
