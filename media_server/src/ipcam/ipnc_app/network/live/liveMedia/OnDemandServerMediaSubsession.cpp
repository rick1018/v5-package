...
::getStreamParameters(unsigned clientSessionId,
  ...
  //fprintf(stderr, "%s destinations = %d, tcpSocketNum = %d\n", __func__, destinations, tcpSocketNum); // 20160225
...
void OnDemandServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  //fprintf(stderr, "%s clientSessionId = %d, destinations = %d\n", __func__, clientSessionId, destinations); // 20160225
  if (streamState != NULL) {
  ...
  if (fRTCPInstance == NULL && fRTPSink != NULL) {
    ...
    fRTCPInstance
      = RTCPInstance::createNew(fRTPSink->envir(), fRTCPgs,
		...
		//fprintf(stderr, "%s createNew RTCPInstance = %d\n", __func__, fRTCPInstance); // 20160225
  }

  if (dests->isTCP) {
    // Change RTP and RTCP to use the TCP socket instead of UDP:
	//fprintf(stderr, "%s ##### TCP fRTCPInstance = %d, fRTPSink = %d\n", __func__, fRTCPInstance, fRTPSink); // 20160225
    if (fRTPSink != NULL) {
	...
	//fprintf(stderr, "%s ##### UDP fRTCPInstance = %d \n", __func__, fRTCPInstance); // 20160225
