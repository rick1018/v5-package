...
#endif
int reset_flag = 0;
int audio_reset_flag = 0;
int agent_qt = 0;
int os_mac = 0;
int agent_seenergy = 0;
...
//fprintf(stderr, "RTSPServer::createNew\n");
...
//  fprintf(stderr, "incomingConnectionHandlerRTSP\n");
  server->incomingConnectionHandlerRTSP1();
}
void RTSPServer::incomingConnectionHandlerRTSP1() {
	fprintf(stderr, "incomingConnectionHandlerRTSP1\n");// 20160225
...
//	fprintf(stderr, "incomingConnectionHandlerHTTP1\n");
...
//  fprintf(stderr, "incomingConnectionHandler clientSocket = %d, addr = %s\n", clientSocket,
//  		inet_ntoa(clientAddr.sin_addr));
  makeSocketNonBlocking(clientSocket);
  increaseSendBufferTo(envir(), clientSocket, 50*1024);
 // increaseSendBufferTo(envir(), clientSocket, 64*1024);
...
printf("%s(%d)\n", __func__, __LINE__);
  ...
  resetRequestBuffer();
printf("%s(%d)\n", __func__, __LINE__);
  fprintf(stderr, "%s %d, clientSocket = %d\n", __func__, this, clientSocket);
...
//	fprintf(stderr, "RTSPServer::~RTSPClientSession %d, sessionId = %d, iSock= %d\n", this, fClientInputSocket);
  closeSockets();
  ...
  if (fSessionCookie != NULL) {
	...
	fprintf(stderr, "%s %d, fClientInputSocket = %d #####\n", __func__, this, fClientInputSocket);// 20160225
  reclaimStreamStates();
...
void RTSPServer::RTSPClientSession::closeSockets() {	
  ...
  fprintf(stderr, "%s $$$$$ %d\n", __func__, fClientInputSocket);// 20160225
...
void RTSPServer::RTSPClientSession::handleRequestBytes(int newBytesRead) {
  ...
  do {
//  	fprintf(stderr, "%s 11111 %d socket = %d, %d %d\n", __func__, fNumStreamStates, fClientInputSocket, newBytesRead, fRequestBufferBytesLeft);
    noteLiveness();
...
#endif
//  	fprintf(stderr, "%s 22222 socket = %d, %d\n", __func__, fClientInputSocket, fClientOutputSocket);
...
//Add by Joseph for onvif RTSS-1-1-15 LINE BREAKS IN BASE64 ENCODING
    ptr[newBytesRead] = '\0';
    unsigned char des[newBytesRead];
	memset(des,0,newBytesRead);
    unsigned char *p = ptr; 
	unsigned char *dest = des;
	int kk=0;
    while (*p != '\0') {
     if (*p == '\r' || *p == '\n') {
       p++;
	...
	//*dest++='\0';
	newBytesRead -= kk;
	memcpy(ptr,des,newBytesRead);
//END Add by Joseph for onvif RTSS-1-1-15 LINE BREAKS IN BASE64 ENCODING
...
//	  fprintf(stderr, "%s 33333 %s socket = %d\n", __func__, cmdName, fClientInputSocket);
...
::handleCmd_DESCRIBE(char const* cseq,
	...
	do {
		...
		if (!authenticationOK("DESCRIBE", cseq, urlTotalSuffix, fullRequestStr))
			break;
		if (strstr(fullRequestStr, "os=Mac") != NULL)
			os_mac = 1;
		else
			os_mac = 0;
		if (strstr(fullRequestStr, "QuickTime") != NULL)
			agent_qt = 1;
		else
			agent_qt = 0;
		if (strstr(fullRequestStr, "SEEnergy") != NULL)
			agent_seenergy = 1;
		else
			agent_seenergy = 0;
...
::handleCmd_SETUP(char const* cseq,
  ...
  //    "urlPreSuffix" concatenated with "urlSuffix" (with "/" inbetween) is the session (stream) name.
	...
	do {
		...
		/* Quick Time - 20130402 - Dean - Begin */
		// Add the indication of timeout (from RFC 2326 section 12.37) if we are reclaiming streams.
		// Note: Quicktime does not respect the 60 second default RTSP timeout. It sends nothing
...
#define RESPONSE_TIMEOUT
		...
		if (timeout > 0)
			snprintf(timeoutAttributeStr, sizeof timeoutAttributeStr, ";timeout=%d", timeout);
		else
			timeoutAttributeStr[0] = '\0';
#endif
		/* Quick Time - 20130402 - Dean - End */
...
#if 0 // def RESPONSE_TIMEOUT
					"Session: %08X%s\r\n\r\n",			/* Quick Time - 20130402 - Dean */
#else
					"Session: %08X\r\n\r\n",			/* Quick Time - 20130402 - Dean */
#endif
					cseq,
...
#if 0 // def RESPONSE_TIMEOUT
					fOurSessionId, timeoutAttributeStr);	/* Quick Time - 20130402 - Dean */
#else
					fOurSessionId); 						/* Quick Time - 20130402 - Dean */
#endif
				break;
			case RTP_TCP:
...
void RTSPServer::RTSPClientSession
  ...
  rangeHeader = strDup(buf);
	if (subsession == NULL)// 20160225
		fprintf(stderr, "%s &&&&& subsession is NULL %d\n", __func__, fNumStreamStates);
	else
	  fprintf(stderr, "%s &&&&& %d %d\n", __func__, subsession, fNumStreamStates);
  // Now, start streaming:
  for (i = 0; i < fNumStreamStates; ++i) {
    if (subsession == NULL /* means: aggregated operation */
	|| subsession == fStreamStates[i].subsession) {
	  ...
	  //fprintf(stderr, "%s @@@@@ %d %d\n", __func__, fClientInputSocket, fStreamStates[i].subsession); // 20160225
...
#if 0
  char *findqt=NULL;
  findqt = strstr(reqStr, "QuickTime");
  if (findqt!=NULL)
  {
  	return False;
  }
...
void RTSPServer::RTSPClientSession::handleHTTPCmd_TunnelingGET(char const* sessionCookie) {
  ...
  //fprintf(stderr, "handleHTTPCmd_TunnelingGET\n");
#ifdef DEBUG
...
::handleHTTPCmd_TunnelingPOST(char const* sessionCookie, unsigned char const* extraData, unsigned extraDataSize) {
  ...
  //fprintf(stderr, "handleHTTPCmd_TunnelingPOST\n");
...
::noteClientLiveness(RTSPClientSession* clientSession) {
	//fprintf(stderr, "%s @@@@@  %d\n", __func__, clientSession->fClientInputSocket); // 20160225
...
::changeClientInputSocket(int newSocketNum, unsigned char const* extraData, unsigned extraDataSize) {
  envir().taskScheduler().turnOffBackgroundReadHandling(fClientInputSocket);
  //fprintf(stderr, "%s @@@@@  %d %d\n", __func__, fClientInputSocket ,newSocketNum);
