...
::setSpecificRRHandler(netAddressBits fromAddress, Port fromPort,
  ...
  void *pVoid = fSpecificRRHandlerTable->Add(fromAddress, (~0), fromPort, rrHandler);
  //fprintf(stderr, "%s ##### %d %d\n", __func__, rrHandler, ntohs(fromPort.num())); // 20160225
}
...
::unsetSpecificRRHandler(netAddressBits fromAddress, Port fromPort) {
  ...
  //fprintf(stderr, "%s ##### %d %d\n", __func__, rrHandler, rrHandler->rrHandlerClientData); // 20160225
  if (rrHandler != NULL) {
...
void RTCPInstance::setStreamSocket(int sockNum,
  ...
  TaskScheduler::BackgroundHandlerProc* handler
    = (TaskScheduler::BackgroundHandlerProc*)&incomingReportHandler;
  //fprintf(stderr, "%s sockNum = %d, streamChannelId = %d, fRTCPInterface = %d, handler = %d\n", __func__, sockNum, streamChannelId, &fRTCPInterface, handler); // 20160225
  fRTCPInterface.startNetworkReading(handler);
}
...
void RTCPInstance::addStreamSocket(int sockNum,
  ...
  //fprintf(stderr, "%s turnOffBackgroundReadHandling sockNum = %d\n", __func__, fRTCPInterface.gs()->socketNum()); // 20160225
...
//fprintf(stderr, "%s RTCPInstance = %d SocketNum = %d, packetSize = %d\n", __func__, this, tcpReadStreamSocketNum, packetSize);
		...
		= (RRHandlerRecord*)(fSpecificRRHandlerTable->Lookup(fromAddr, (~0), fromPort));
	      if (rrHandler != NULL) {
			if (rrHandler->rrHandlerTask != NULL) {
			  ...
			  //fprintf(stderr, "%s RTCPInstance = %d rrHandler = %d, %d, port = %d\n", __func__, this, rrHandler, tcpReadStreamSocketNum, ntohs(fromPort.num())); // 20160225
			}
	      }
	    }
