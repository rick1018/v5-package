...
int is_ssm = 0; // add by kenny chiu 20120113
extern int os_mac;
extern int agent_qt;
////////// MultiFramedRTPSink //////////
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  //  setPacketSizes(1024*8,8192);  

#if 1
	if (is_ssm)
	{
		setPacketSizes(1024<<3,1024<<3);
		//setPacketSizes(1024, 1448);
	}
	else
	{
		if (os_mac == 1/* || agent_qt == 1*/)
			//setPacketSizes(1024, 1450);
			setPacketSizes(1024, 1448);
		else
		{
			//fprintf(stderr, "rtpPayloadType = %d\n", rtpPayloadType);
			if (rtpPayloadType == 0) // audio
				setPacketSizes(1024*2,1024*2);
			else
				setPacketSizes(1024*8,1024*8);
		}
	}
#else
	setPacketSizes(1024, 1450);
