...
/*	struct timeval timeout;
	timeout.tv_sec=RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS/1000;
	timeout.tv_usec=(RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS%1000)*1000;
	setsockopt(fGS->socketNum(),SOL_SOCKET,SO_SNDTIMEO,&timeout,sizeof(timeout)); 
	setsockopt(fGS->socketNum(),SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout)); 	
	makeSocketBlocking(fGS->socketNum());
*/ 
...
void RTPInterface::addStreamSocket(int sockNum,
  ...
  //added by frank for blocking mode always.
	/*struct timeval timeout;
	timeout.tv_sec=RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS/1000;
	timeout.tv_usec=(RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS%1000)*1000;
	setsockopt(sockNum,SOL_SOCKET,SO_SNDTIMEO,&timeout,sizeof(timeout)); 
	setsockopt(sockNum,SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout)); 
	makeSocketBlocking(sockNum);*/
	increaseSendBufferTo(envir(), sockNum, 800*1024);	
...
static void deregisterSocket(UsageEnvironment& env, int sockNum, unsigned char streamChannelId) {
  ...
  if (socketDescriptor != NULL) {
	...
	socketDescriptor->setServerRequestAlternativeByteHandler(NULL, NULL);
	//fprintf(stderr, "deregisterSocket\n");
...
::startNetworkReading(TaskScheduler::BackgroundHandlerProc* handlerProc) {
  ...
  envir().taskScheduler().
   turnOnBackgroundReadHandling(fGS->socketNum(), handlerProc, fOwner);
	//fprintf(stderr, "%s sock = %d, handlerProc = %d %d\n", __func__, fGS->socketNum(), handlerProc, fOwner); 
  ...
  for (tcpStreamRecord* streams = fTCPStreams; streams != NULL;
	...
	//added by frank for blocking mode.
	/*struct timeval timeout;
	timeout.tv_sec=RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS/1000;
	timeout.tv_usec=(RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS%1000)*1000;
	setsockopt(streams->fStreamSocketNum,SOL_SOCKET,SO_SNDTIMEO,&timeout,sizeof(timeout)); 
	setsockopt(streams->fStreamSocketNum,SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout)); 	
	makeSocketBlocking(streams->fStreamSocketNum);*/
...
void RTPInterface::stopNetworkReading() {
  ...
  fprintf(stderr, "%s sock = %d, %d\n", __func__, fGS->socketNum(), fOwner); 
...
#if 0 // modify by Kenny Chiu 20150529 per frank.lin
Boolean RTPInterface::sendRTPOverTCP(u_int8_t* packet, unsigned packetSize,
  ...
  do {
	...
	//memcpy(framingHeader+4, packet, packetSize);
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed) {
  ...
  if (sendResult < (int)dataSize && forceSendToSucceed) {
...
//#ifdef DEBUG_SEND
      fprintf(stderr, "sendDataOverTCP: socketNum:%d resending %d-byte send (blocking)\n", socketNum, numBytesRemainingToSend); fflush(stderr);
//#endif

     // makeSocketBlocking(socketNum);//, RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS);
     //if have error, then remove it.
	 // removeStreamSocket(socketNum, 0xFF);
	struct timeval timeout;
	timeout.tv_sec=RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS/1000;
	timeout.tv_usec=(RTPINTERFACE_BLOCKING_WRITE_TIMEOUT_MS%1000)*1000;
	setsockopt(socketNum,SOL_SOCKET,SO_SNDTIMEO,&timeout,sizeof(timeout)); 
	setsockopt(socketNum,SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout)); 
	makeSocketBlocking(socketNum);
	 
	int totalSent =0;
	char *data1 = (char*)(data+numBytesSentSoFar);
	unsigned dataLen = numBytesSentSoFar;
	while(totalSent < numBytesRemainingToSend)
	{
		int bytesSent = send(socketNum, (char const*)data+totalSent, numBytesRemainingToSend-totalSent, 0);
		if (bytesSent == 0)
		{
			return True;
		}
	...
	if(totalSent<numBytesRemainingToSend) 
		{
			fprintf(stderr, "after removeStreamSocket\n"); fflush(stderr);	
			removeStreamSocket(socketNum, 0xFF);	
			return False;
		}
	else 
...
Boolean RTPInterface::sendRTPOverTCP(u_int8_t* packet, unsigned packetSize,
  ...
  do {
	...
	if(select(socketNum+1, 0, &wrts, NULL, &timeout)<=0)
	{
	  // fprintf(stderr, "sendRTPOverTCP1:select failed\n"); fflush(stderr);
	   if(envir().getErrno() == EAGAIN || envir().getErrno() == EWOULDBLOCK||envir().getErrno()==EINTR)
	   {
			fprintf(stderr, "sendRTPOverTCP Error 11111111\n");
			return 0;
	   }
	   else
	   {
	   	   	fprintf(stderr, "sendRTPOverTCP Error 22222222 socketNum = %d\n", socketNum);
			envir().taskScheduler().turnOffBackgroundReadHandling(socketNum); // stops further calls to us
			removeStreamSocket(socketNum, 0xFF);
...
::deregisterRTPInterface(unsigned char streamChannelId) {
  ...
  if (fSubChannelHashTable->IsEmpty()) {
    ...
    removeSocketDescription(fEnv, fOurSocketNum);  
	delete this;
	fprintf(stderr, "SocketDescriptor11(socket %d)::deregisterRTPInterface(channel %d)\n", fOurSocketNum, streamChannelId);
#if 0  
...
void SocketDescriptor::tcpReadHandler(SocketDescriptor* socketDescriptor, int mask) {
 ...
 socketDescriptor->tcpReadHandler1(mask);
  ...
  //	delete socketDescriptor;
	//fprintf(stderr, "tcpReadHandler,  socketDescriptor->fAreInReadHandlerLoop = False\n");
  }
}
