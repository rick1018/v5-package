...
Boolean parseRTSPRequestString(char const* reqStr,
  ...
  //added by frank, for skip ".sdp" string. 2015/07/28 for AcelinkViewII
//  fprintf(stderr, "%s @@@@@ resultURLSuffix = %s, resultCmdName = %s\n", __func__, resultURLSuffix, resultCmdName);
  if(strlen(resultURLSuffix)>4 && resultURLSuffix[strlen(resultURLSuffix)-4]=='.' && resultURLSuffix[strlen(resultURLSuffix)-3]=='s'
  	&& resultURLSuffix[strlen(resultURLSuffix)-2]=='d' && resultURLSuffix[strlen(resultURLSuffix)-1]=='p')
  		resultURLSuffix[strlen(resultURLSuffix)-4]='\0';
