...
RTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  if (ourSocket == -1) return NULL;
	int t = 1;
	fprintf(stderr, "RTSPServer::createNew ourSocket = %d, ourPort = %d\n", ourSocket, ourPort.num());
	setsockopt(ourSocket, SOL_SOCKET, SO_REUSEADDR, &t, sizeof(t));  
...
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr),
	...
	//fprintf(stderr, "%s %s:%d, sock = %d\n", __func__, inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port), clientSocket);
  resetRequestBuffer();
}
...
//	else
//		agent_seenergy = 0;
	if (strstr(fullRequestStr, "LIVE555 Streaming Media") != NULL)
		agent_live555 = 1;
	else
		agent_live555 = 0;
...
//  fprintf(stderr, "%s fClientOutputSocket = %d, fClientInputSocket = %d\n", __func__, fClientOutputSocket, fClientInputSocket);
...
#endif
//	fprintf(stderr, "%s playAfterSetup = %d, fResponseBuffer = %s\n", __func__, playAfterSetup, fResponseBuffer);
...
//    fprintf(stderr, "RTSPClientSession clientSocket = %d\n", clientSocket);
}
...
void RTSPServer::RTSPClientSession::reclaimStreamStates() {
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
		fprintf(stderr, "reclaimStreamStates tcpSocketNum = %d\n", fStreamStates[i].tcpSocketNum);
...
#if 0 // def RESPONSE_TIMEOUT
			fOurSessionId, timeoutParameterString);	/* Quick Time - 20130402 - Dean */
#else
